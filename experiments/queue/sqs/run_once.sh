...
ssh -q $CLIENT_HOST -- docker run -v /tmp:/tmp \
    ...
    cp /queuebench-bin/init_queues /tmp/init_queues

QUEUE_PREFIX=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 8 | head -n 1)

ssh -q $CLIENT_HOST -- /tmp/init_queues \
    --faas_gateway=$ENTRY_HOST:8080 --fn_prefix=sqs \
    --queue_prefix=$QUEUE_PREFIX --num_queues=1
